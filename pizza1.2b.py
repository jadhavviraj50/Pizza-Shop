...
collection_queue_lock = threading.Lock()
...
# Define a function to simulate tasks
def simulate_task(task_name, duration):
    time.sleep(duration)
    return f"{task_name} completed"

# Define a function to process tasks from the task queue
def process_tasks():
    while True:
        task = task_queue.get()
        if task is None:
...
def update_result_label(result):
    ...
    new_text = current_text + result + "\n"
    app.after(0, lambda: result_label.config(text=new_text))
        
def load_orders_from_json(file_path):
    with open(file_path, "r") as json_file:
        orders = json.load(json_file)
        for order_id, pizza_names in orders.items():
            task_queue.put((f"Order {order_id}", 2))  # Simulate task with 2 seconds duration
...
def start_simulation():
    simulation_thread = threading.Thread(target=simulate_workflow)
    simulation_thread.start()

def simulate_workflow():
    for i in range(1, 31):
        simulation_progress = f"Order {i} in progress..."
        print(simulation_progress)  # Print to the console
        app.after(0, lambda text=simulation_progress: update_simulation_textbox(text))
        time.sleep(2)  # Simulate task duration
        simulation_completed = f"Order {i} completed"
        print(simulation_completed)  # Print to the console
        app.after(0, lambda text=simulation_completed: update_simulation_textbox(text))

def update_simulation_textbox(text):
    simulation_textbox.insert(tk.END, text + "\n")
    simulation_textbox.see(tk.END)  # Scroll to the end of the text box
...
result_label.grid(row=12, column=0, columnspan=4, padx=10, pady=10)

# Create a button to start the simulation
simulate_button = tk.Button(app, text="Simulate Workflow", command=start_simulation)
simulate_button.grid(row=13, column=0, columnspan=4, padx=10, pady=10)

# Create a text box to display the simulation progress
simulation_textbox = tk.Text(app, height=10, width=50)
simulation_textbox.grid(row=10, column=5, columnspan=4, padx=10, pady=10)



# Update the GUI and perform other tasks
def update_gui():
    update_timer()
    update_collection_queue_display()
...
num_worker_threads = 3  # You can adjust the number of worker threads
for _ in range(num_worker_threads):
    worker_thread = threading.Thread(target=process_tasks)
    worker_thread.daemon = True
    worker_thread.start()

# Load orders from JSON file and add to the task queue
load_orders_from_json("orders.json")
...
# Signal the worker threads to stop when all tasks are done
for _ in range(num_worker_threads):
    task_queue.put(None)
